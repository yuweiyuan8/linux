		  ...
		  int dataSize)
{
	...
	if (addrSize <= sizeof(u64)) {
		finalCmd = kmalloc(sizeof(u8) * (1 + addrSize + WRITE_CHUNK), GFP_KERNEL);
		while (remaining > 0) {
...
{
	u8 *finalCmd = kmalloc(sizeof(u8) * (1 + addrSize), GFP_KERNEL);
	...
	while (remaining > 0) {
		...
		if (hasDummyByte == 1) {
			if (fts_writeRead(finalCmd, 1 + addrSize, buff,
					  toRead + 1) < OK) {
				logError(1, "%s %s: read error... ERROR %08X\n",
					 tag, __func__, ERROR_BUS_WR);
				kfree(finalCmd);
				return ERROR_BUS_WR;
			}
...
int fts_writeU8UXthenWriteU8UX(u8 cmd1, AddrSize addrSize1, u8 cmd2,
			       ...
			       int dataSize)
{
	u8 *finalCmd1 = kmalloc(sizeof(u8) * (1 + addrSize1), GFP_KERNEL);
	u8 *finalCmd2 = kmalloc(sizeof(u8) * (1 + addrSize2 + WRITE_CHUNK), GFP_KERNEL);
	int remaining = dataSize;
	int toWrite = 0, i = 0;
	...
	while (remaining > 0) {
		...
		if (fts_write(finalCmd2, 1 + addrSize2 + toWrite) < OK) {
			logError(1, "%s %s: second write error... ERROR %08X\n",
				 tag, __func__, ERROR_BUS_W);
			kfree(finalCmd1);
			kfree(finalCmd2);
			return ERROR_BUS_W;
...
{
	u8 *finalCmd1 = kmalloc(sizeof(u8) * (1 + addrSize1), GFP_KERNEL);
	u8 *finalCmd2 = kmalloc(sizeof(u8) * (1 + addrSize2), GFP_KERNEL);
	u8 buff[READ_CHUNK + 1]; /* worst case has dummy byte */
	int remaining = byteToRead;
	int toRead = 0, i = 0;
