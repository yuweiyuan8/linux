...
short **array1dTo2d_short(short *data, int size, int columns)
{
	...
	if (size == 0) {
		...
		matrix = (short **)kmalloc(
			((int)(size / columns)) * sizeof(short *), GFP_KERNEL);

		if (matrix != NULL) {
			for (i = 0; i < (int)(size / columns); i++)
				matrix[i] = (short *)kmalloc(
					columns * sizeof(short), GFP_KERNEL);
...
u16 **array1dTo2d_u16(u16 *data, int size, int columns)
{
	...
	if (size == 0) {
		...
		matrix = (u16 **)kmalloc(
			((int)(size / columns)) * sizeof(u16 *), GFP_KERNEL);

		if (matrix != NULL) {
			for (i = 0; i < (int)(size / columns); i++)
				matrix[i] = (u16 *)kmalloc(
					columns * sizeof(u16), GFP_KERNEL);
