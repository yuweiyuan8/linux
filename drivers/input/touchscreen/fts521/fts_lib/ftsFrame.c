...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
static int readSyncDataHeader(u8 type, DataHeader *msHeader,
...
int getSyncFrame(u8 type, MutualSenseFrame *msFrame, SelfSenseFrame *ssFrame)
{
	...
	msFrame->node_data_size =
		msFrame->header.force_node * msFrame->header.sense_node;

	msFrame->node_data = (short *)kmalloc(
		msFrame->node_data_size * sizeof(short), GFP_KERNEL);
	...
	ssFrame->force_data = (short *)kmalloc(
		ssFrame->header.force_node * sizeof(short), GFP_KERNEL);
	...
	ssFrame->sense_data = (short *)kmalloc(
		ssFrame->header.sense_node * sizeof(short), GFP_KERNEL);
